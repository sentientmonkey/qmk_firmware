...
void keyboard_post_init_user(void) {
    ...
    for (uint16_t i = 0; i < 256; ++i) {
        rgblight_sethsv( (i + user_hue) % 256, 255, 255);
        wait_ms(5);
     }
     layer_state_set_user(layer_state);
};
...
layer_state_t layer_state_set_user(layer_state_t state) {
    switch (biton32(state)) {
        case _QWERTY: rgblight_sethsv(HSV_GREEN); break;
        case _LOWER: rgblight_sethsv(HSV_BLUE); break;
        case _RAISE: rgblight_sethsv(HSV_RED); break;
        default: rgblight_sethsv(HSV_WHITE); break;
    }
    return state;
};
